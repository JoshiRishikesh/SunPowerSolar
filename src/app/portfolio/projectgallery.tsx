...
const allProjects: Project[] = [
  {
    ...
    imageUrl: `https://placehold.co/600x400/${BRAND_PRIMARY_DARK.substring(1)}/${BRAND_ACCENT_LIGHT.substring(1)}?text=Miami+Solar`,
    summary: 'A challenging ground-mount installation with high energy demands.',
    details: 'This project featured a customized ground-mount solution to maximize sun exposure on a large suburban property. We used high-efficiency REC panels and a Tesla Powerwall for battery backup, ensuring energy independence during frequent Florida storms. Permitting was completed in a record 3 weeks.',
  },
  {
    id: 2,
    ...
    imageUrl: `https://placehold.co/600x400/${BRAND_PRIMARY_DARK.substring(1)}/${BRAND_ACCENT_LIGHT.substring(1)}?text=Orlando+Commercial`,
    summary: 'Commercial flat-roof system dramatically reducing operating costs.',
    details: 'Installed a large-scale, non-penetrating system on a commercial flat roof. The client saw immediate cost reductions and achieved LEED certification points. We used SolarEdge optimizers to manage shading from adjacent high-rise buildings, maximizing total energy yield.',
  },
  {
    id: 3,
    ...
    imageUrl: `https://placehold.co/600x400/${BRAND_PRIMARY_DARK.substring(1)}/${BRAND_ACCENT_LIGHT.substring(1)}?text=Gainesville+Farm`,
    summary: 'Standard residential roof mount with excellent south-facing orientation.',
    details: 'A straightforward, high-impact installation for a family home. The roof had perfect orientation, allowing for maximum panel efficiency. The system was integrated with a smart home monitoring system, giving the owner real-time performance data via a mobile app.',
  },
  {
    id: 4,
    ...
    imageUrl: `https://placehold.co/600x400/${BRAND_PRIMARY_DARK.substring(1)}/${BRAND_ACCENT_LIGHT.substring(1)}?text=Naples+Luxury`,
    summary: 'Aesthetics-focused installation using black-on-black panels.',
    details: 'We designed a low-profile, sleek system for a luxury home, prioritizing aesthetics. The black-on-black panels blended seamlessly with the tile roof. Due to the coastal location, all mounting hardware was specified for high corrosion resistance.',
  },
];
  ...
  <AnimatePresence>
    ...
    >
      ...
      >
        ...
        <div className="p-8">
          <h2 className="text-3xl font-extrabold mb-4" style={{ color: BRAND_PRIMARY_DARK }}>{project.title}</h2>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 p-4 rounded-xl" style={{ backgroundColor: BRAND_GRAY_LIGHT }}>
          ...
          <div className="flex items-center justify-between p-4 rounded-xl font-semibold text-white bg-gradient-to-r from-yellow-400 via-yellow-300 to-yellow-400">
            ...
            <span className="text-xl font-extrabold text-gray-800">{project.estimatedSavings}</span>
          </div>
